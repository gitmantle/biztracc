...
<br>
  <table width="600" border="0" align="center">
    <tr>
      <td colspan="2"><div align="center" class="style1 style3"><u>Add a Debtor Sub Account</u></div></td>
    </tr>
    <tr>
...
<?php
	...
	if ($ok == 'Y') {
		...
		$db->query("select client_id,drno from ".$cltdb.".client_company_xref where uid = ".$tid);
		$row = $db->single();
		extract($row);
		
		$db->query("select max(drsub) as lastsub from ".$cltdb.".client_company_xref where drno = ".$drno);
		$row = $db->single();
		if (count($row == 0)) {
			$subno = 1;
		} else {
		...
		$db->query("insert into ".$cltdb.".client_company_xref (client_id,company_id,drno,drsub,subname,sortcode) values (:client_id,:company_id,:drno,:drsub,:subname,:sortcode)");
		$db->bind(':client_id', $client_id);
		$db->bind(':company_id', $coyidno);
		$db->bind(':drno', $drno);
		$db->bind(':drsub', $subno);
		$db->bind(':subname', $sname);
		$db->bind(':sortcode', $lastname.$drno."-".$subno);
