...
$sno_ref = $_REQUEST['son'];
$_SESSION['s_salesorderref'] = $sno_ref;
$rno = substr($sno_ref,3);
...
// get header items from invhead
$db->query("select accountno,sub,gldesc,ddate,staff,postaladdress,deliveryaddress,client,currency,rate from ".$findb.".invhead where ref_no = '".$sno_ref."'");
$row = $db->single();
extract($row);
$cry = $currency.'~'.$rate;
$currencycode = $currency;
$ddateh = $ddate;
$debtor = $client.'~'.$accountno.'~'.$sub;

// get clientid
$db->query("select client_id,member from ".$cltdb.".client_company_xref where drno = ".$accountno." and drsub = ".$sub);
$row = $db->single();
extract($row);

// get trading tax type from invtrans
$db->query("select taxtype from ".$findb.".invtrans where ref_no = '".$sno_ref."' limit 1");
$row = $db->single();
extract($row);
...
function addtrdtrans() {
	...
	var stkqty = parseFloat(document.getElementById('qty').value);
	var sent = parseFloat(document.getElementById('sent').value);
	var origqty = parseFloat(document.getElementById('origqty').value);
	if ((stkqty < origqty) && (stkqty > (origqty - sent))) {
		alert('You may not change the quantity to less than the '+sent+' that has/have already been delivered');
		return false;
	} else {
...
function editlineitem(uid) {
	...
	$.get("includes/ajaxgetsoline.php", {lineno: uid}, function(data){
		var ln = data.split('~');
		var itemcode = ln[0];
		var stkitem = ln[1];
		var price = ln[2];
		...
		document.getElementById('origqty').value = qty;
...
function dellineitem(uid) {
	$.get("includes/ajaxgetsoline.php", {lineno: uid}, function(data){
		...
		var sent = ln[18];
		if (sent > 0) {
			alert('Items from this line have been delivered. You may not delete it.');
			return false;
		} else {
			$.get("includes/ajaxdeltrdline.php", {tid: uid}, function(data){$("#solist").trigger("reloadGrid")});
		}
...
<form name="so" id="so" method="post" >
  ...
  <input type="hidden" name="scurrency" id="scurrency" value="<?php echo $cry; ?>">
  <input type="hidden" name="TRaccount" id="TRaccount" value="<?php echo $debtor; ?>">
  <input type="hidden" name="sent" id="sent" value="0">
  <input type="hidden" name="origqty" id="origqty" value="0">
  ...
  <table  width="950" border="0" cellpadding="3" cellspacing="1" align="center" bgcolor="<?php echo $bgcolor; ?>">
    ...
    <tr>
      ...
      <td colspan="8" ><div align="left">
          <input name="description" id="description" type="text" value="<?php echo $gldesc; ?>" readonly size="50" maxlength="60">
        </div></td>
      <td  class="boxlabel">Reference</td>
      <td><input name="ref" id="ref" type="text" size="5" value="S_O" readonly></td>
      <td><input name="refno" type="text" id="refno" size="10" value="<?php echo $rno; ?>" readonly></td>
    </tr>
    <tr>
      <td class="boxlabel" >Currency</td>
      <td ><input name="fxy" id="fxy" type="text" size="5" value="<?php echo $currencycode; ?>" readonly></td>
      <td class="boxlabel">Client</td>
      <td colspan="8" ><input type="text" name="account" id="account" size="45" value="<?php echo $client; ?>" readonly ></td>
      <td class="boxlabel">&nbsp;</td>
      <td colspan="2" >&nbsp;</td>
    </tr>
    <tr>
      <td colspan="14" class="boxlabelleft" >Post Invoice to:
          <input type="text" name="postadd" id="postadd" size="45" value="<?php echo $postaladdress; ?>" readonly >
        &nbsp;&nbsp;&nbsp;&nbsp;
        Deliver Goods to:
        <input type="text" name="deliveradd" id="deliveradd" size="45" value="<?php echo $deliveryaddress; ?>" readonly ></td>
    </tr>
    <tr bgcolor="<?php echo $bghead; ?>">
      <td colspan="13"><label style="color: <?php echo $thfont; ?>"><strong>Add/Edit an Item </strong></label>
        &nbsp;&nbsp;&nbsp;Barcode&nbsp;
