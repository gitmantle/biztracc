...
$grid->SelectCommand = "select uid,ref_no,item,quantity,price,unit,value,tax,(value+tax) as total,paid,(value+tax-paid) - (returns*price*(1+(taxpcent/100))) as outstanding  from invtrans where ref_no = '".$ref."'";
...
$grid->setUrl('getallocgrnlines.php');
...
function(rowid){
	var ids = jQuery("#algrntrans").getDataIDs();
	for(var i=0;i<ids.length;i++){
			var rowd = $("#algrntrans").getRowData(ids[i]);
			var cl = ids[i];
			...
			jQuery("#algrntrans").setRowData(ids[i],{act:be+'&nbsp;&nbsp;&nbsp;'+se}); 
	}
}
LOADCOMPLETE;
...
// Run the script
$grid->renderGrid('#algrntrans','#algrntranspager',true, null, null,true,true,true);
?>
