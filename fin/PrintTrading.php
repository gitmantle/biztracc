...
$dbp->query("select * from ".$findb.".invhead where ref_no = :tradingref");
...
if ($type == 'INV') {
	...
	$dbp->query("select ".$cltdb.".comms.comm from ".$cltdb.".comms,".$cltdb.".client_company_xref where ".$cltdb.".comms.member_id = ".$cltdb.".client_company_xref.client_id and ".$cltdb.".comms.comms_type_id = 4 and ".$cltdb.".client_company_xref.drno = :draccno and ".$cltdb.".client_company_xref.drsub = :drsubno");
	$dbp->bind(':draccno', $draccno);
	$dbp->bind(':drsubno', $drsubno);
	$row = $dbp->single();
	extract($row);
...
//******************************************************************************
{
	global $template, $pdf, $lastpage, $currentline, $tradingref, $findb, $dbp, $type, $fxcode, $fxrate;
	
	if ($type == 'PKL') {
		$dbp->query("select itemcode,item,unit,(quantity - returns) as required,' ' from ".$findb.".invtrans where ref_no = :tradingref");
		$dbp->bind(':tradingref', $tradingref);
		$linedetails = $dbp->resultsetNum();
	} else {
		if ($type == 'D_N') {
			$dbp->query("select itemcode,item,quantity,unit from ".$findb.".invtrans where ref_no = :tradingref");
			$dbp->bind(':tradingref', $tradingref);
			$linedetails = $dbp->resultsetNum();
		} else {
			$dbp->query("select itemcode,item,quantity,(price*rate) as price,(value*rate) as value,(discount*rate) as discount,(tax*rate) as rate,(value+tax-discount)*rate as total from ".$findb.".invtrans where ref_no = :tradingref");
			$dbp->bind(':tradingref', $tradingref);
			$linedetails = $dbp->resultsetNum();
		}
	}
			...
			if ($pdf->PageNo() == 1) {
				...
				for ($i = $currentline; $i < $numlines; $i++) {
					...
					for ($n = 0; $n <= $gcount; $n++) {
						...
						// sort out forex
						if ($type == 'INV' && ($n >= 3 && $n <= 7)) {
							$texte = number_format($rowl[$n]*$fxrate,2);
						} else {
...
//********************************************************************************
{
	global $template, $pdf, $reference, $tradingref, $remmitance, $signature, $findb, $dbp, $fxcode, $fxrate;
	
	$dbp->query("select sum(totvalue*rate) as totvalue, sum(tax*rate) as tax, currency from ".$findb.".invhead where ref_no = :tradingref");
	$dbp->bind(':tradingref', $tradingref);
	$row = $dbp->single();
	extract($row);
	$totalvalue = $totvalue;
...
//******************************************************************************************************
	...
	if ($ok == 'Y') {
		...
		$subject = ($doctype.' '.$fname);
		$mess = $doctype." ".$fname." attached\r\n\r\n"."Regards\r\n".$coyname;
		$to = trim($email_to);
