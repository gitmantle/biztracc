...
$grid->SelectCommand = "select uid,seq,title,include from ".$findb.".".$tfile." where tab = 'system'";
...
$grid->setUrl('../fin/getfindocsystem.php');
...
function(rowid){
	var ids = jQuery("#fdocsystemlist").getDataIDs();
	for(var i=0;i<ids.length;i++){
			var rowd = $("#fdocsystemlist").getRowData(ids[i]);
			var cl = ids[i];
			be = '<img src="../images/edit.png" title="Edit Properties" onclick="javascript:editproperties('+cl+')" ></ids>';
			jQuery("#fdocsystemlist").setRowData(ids[i],{act:be}); 
	}
}
LOADCOMPLETE;
...
// Run the script
$grid->renderGrid('#fdocsystemlist','#fdocsystempager',true, null, null,true,true,true);
