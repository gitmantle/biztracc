...
$grid->SelectCommand = "select uid,seq,title,include from ".$findb.".".$tfile." where tab = 'box'";
...
$grid->setUrl('../fin/getfindocboxes.php');
...
function(rowid){
	var ids = jQuery("#fdocboxeslist").getDataIDs();
	for(var i=0;i<ids.length;i++){
			var rowd = $("#fdocboxeslist").getRowData(ids[i]);
			var cl = ids[i];
			be = '<img src="../images/edit.png" title="Edit Properties" onclick="javascript:editproperties('+cl+')" ></ids>';
			jQuery("#fdocboxeslist").setRowData(ids[i],{act:be}); 
	}
}
LOADCOMPLETE;
...
// Run the script
$grid->renderGrid('#fdocboxeslist','#fdocboxespager',true, null, null,true,true,true);
