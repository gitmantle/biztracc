...
$grid->SelectCommand = "select uid,item,include from ".$findb.".".$tfile." where item in ('gridtitle','griddetail')";
...
$grid->setUrl('../fin/getfindocdetails.php');
...
function(rowid){
	var ids = jQuery("#fdocdetaillist").getDataIDs();
	for(var i=0;i<ids.length;i++){
			var rowd = $("#fdocdetaillist").getRowData(ids[i]);
			var cl = ids[i];
			be = '<img src="../images/edit.png" title="Edit Properties" onclick="javascript:editproperties('+cl+')" ></ids>';
			jQuery("#fdocdetaillist").setRowData(ids[i],{act:be}); 
	}
}
LOADCOMPLETE;
...
// Run the script
$grid->renderGrid('#fdocdetaillist','#fdocdetailpager',true, null, null,true,true,true);
