...
if ($sub = 0) {
	$db->query("select drno,drsub from ".$cltdb.".client_company_xref where (uid = ".$tid." and drsub > 0)");
	$rows = $db->resultset();
...
$db->query("select drno,drsub from ".$cltdb.".client_company_xref where uid = ".$tid);
$row = $db->single();
extract($row);

$db->query("select accountno from ".$findb.".trmain where (accountno = ".$drno." and sub = ".$drsub.")");
$rows = $db->resultset();
