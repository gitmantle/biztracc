...
foreach ($rows as $row) {
	...
	if ($acc2dr > 30000000) {
		$db->query("select member from ".$cltdb.".client_company_xref where drno = ".$acc2dr." and drsub = ".$subdr);
		$mem = $db->single();
	}
	if ($acc2cr > 30000000) {
		$db->query("select member from ".$cltdb.".client_company_xref where drno = ".$acc2cr." and drsub = ".$subcr);
		$mem = $db->single();
	}
	if ($acc2dr > 20000000 && $acc2dr < 30000001) {
		$db->query("select member from ".$cltdb.".client_company_xref where crno = ".$acc2dr." and crsub = ".$subdr);
		$mem = $db->single();
	}
	if ($acc2cr > 20000000 && $acc2cr < 30000001) {
		$db->query("select member from ".$cltdb.".client_company_xref where crno = ".$acc2cr." and crsub = ".$subcr);
		$mem = $db->single();
	}
	...
	switch ($dacctype) {
		case 'din':
			...
			if ($tax > 0) {
				...
				if ($gstinvpay == 'Invoice') {
					...
					$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,gstrecon,grosssales,descript1,taxpcent,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:gstrecon,:grosssales,:descript1,:taxpcent,:your_ref)");
					$db->bind(':ddate', $ddate);
		...
		case 'dex':
			...
			if ($tax > 0) {
				...
				if ($gstinvpay == 'Invoice') {
					$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,gstrecon,grosspurchases,descript1,taxpcent,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:gstrecon,:grosspurchases,:descript1,:taxpcent,:your_ref)");
					$db->bind(':ddate', $ddate);
					$db->bind(':accountno', 870);	//debit GST Payable
					$db->bind(':branch', $br2dr);
					$db->bind(':accno', $acc2cr);
		...
		case 'dbs':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,sub,accno,br,subbr,debit,credit,reference,gsttype,gstrecon,descript1,taxpcent,currency,rate,consign,your_ref) values (:ddate,:accountno,:branch,:sub,:accno,:br,:subbr,:debit,:credit,:reference,:gsttype,:gstrecon,:descript1,:taxpcent,:currency,:rate,:consign,:your_ref)");
			$db->bind(':ddate', $ddate);
			...
			if ($acc2dr == 870) {
				$db->query("update ".$findb.".trmain set grosspurchases = ".$total." where uid = ".$lid);
				$db->execute();
			}
		...
		case 'das':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,subbr,debit,credit,reference,gsttype,descript1,taxpcent,currency,rate,consign,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:subbr,:debit,:credit,:reference,:gsttype,:descript1,:taxpcent,:currency,:rate,:consign,:your_ref)");
			$db->bind(':ddate', $ddate);
			...
			break;
		case 'd50':
			$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,subbr,debit,credit,reference,gsttype,descript1,taxpcent,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:subbr,:debit,:credit,:reference,:gsttype,:descript1,:taxpcent,:your_ref)");
			$db->bind(':ddate', $ddate);
		...
		case 'dcr':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,sub,accno,br,subbr,debit,credit,reference,grn,gsttype,descript1,taxpcent,currency,rate,consign,your_ref) values (:ddate,:accountno,:sub,:accno,:br,:subbr,:debit,:credit,:reference,:grn,:gsttype,:descript1,:taxpcent,:currency,:rate,:consign,:your_ref)");
			$db->bind(':ddate', $ddate);
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,descript1,taxpcent,currency,rate,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:descript1,:taxpcent,:currency,:rate,:your_ref)");
			$db->bind(':ddate', $ddate);
			...
			default:
				...
				$db->query("insert into ".$findb.".invhead (ddate,accountno,branch,sub,gldesc,transtype,ref_no,totvalue,tax,client,staff,cash,cheque,eftpos,ccard,currency,rate) values (:ddate,:accountno,:branch,:sub,:gldesc,:transtype,:ref_no,:totvalue,:tax,:client,:staff,:cash,:cheque,:eftpos,:ccard,:currency,:rate)");
				$db->bind(':ddate', $ddate);
				$db->bind(':accountno', $acc2dr);	
				$db->bind(':branch', $br2cr);
				$db->bind(':sub', $sub2dr);
				...
				$db->query("insert into ".$findb.".invtrans (item,price,quantity,taxtype,taxpcent,tax,ref_no,value,currency,rate) values (:item,:price,:quantity,:taxtype,:taxpcent,:tax,:ref_no,:value,:currency,:rate)");
...
*/
		...
		case 'ddr':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,sub,accno,br,subbr,debit,credit,reference,gsttype,descript1,taxpcent,currency,rate,consign,your_ref) values (:ddate,:accountno,:sub,:accno,:br,:subbr,:debit,:credit,:reference,:gsttype,:descript1,:taxpcent,:currency,:rate,:consign,:your_ref)");
			$db->bind(':ddate', $ddate);
			$db->bind(':accountno', $acc2dr);	//debit the debtor
			$db->bind(':sub', $sub2dr);
			$db->bind(':accno', $acc2cr);
	...
	if ($gstinvpay == 'Payment') {
			...
			if ($tax > 0) {
				...
				if ($acc2dr >= 751 && $acc2dr <= 800) {
					$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,gstrecon,descript1,taxpcent,currency,rate,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:gstrecon,:descript1,:taxpcent,:currency,:rate,:your_ref)");
					$db->bind(':ddate', $ddate);
					...
					$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,gstrecon,descript1,taxpcent,currency,rate,grosssales,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:gstrecon,:descript1,:taxpcent,:currency,:rate,:grosssales,:your_ref)");
					$db->bind(':ddate', $ddate);
	...
	switch ($cacctype) {
		...
		case 'cbs':
			...
			if ($acc2cr == 870) {
				$db->query("update ".$findb.".trmain set grosssales = ".$total." where uid = ".$lid);
				$db->execute();
			}			
		...
		case 'cas':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,subbr,debit,credit,reference,gsttype,descript1,taxpcent,currency,rate,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:subbr,:debit,:credit,:reference,:gsttype,:descript1,:taxpcent,:currency,:rate,:your_ref)");
			$db->bind(':ddate', $ddate);
		...
		case 'ccr':
			...
			//if ($payrec <> 'PAY' && $payrec <> 'REC') {
				...
				$db->query("insert into ".$findb.".invhead (ddate,accountno,branch,sub,gldesc,transtype,ref_no,totvalue,tax,client,staff,currency,rate) values (:ddate,:accountno,:branch,:sub,:gldesc,:transtype,:ref_no,:totvalue,:tax,:client,:staff,:currency,:rate)");
				$db->bind(':ddate', $ddate);
				$db->bind(':accountno', $acc2cr);	
				$db->bind(':branch', $br2dr);
				$db->bind(':sub', $sub2cr);
		...
		case 'cdr':
			...
			$db->query("insert into ".$findb.".trmain (ddate,accountno,sub,accno,br,subbr,debit,credit,reference,inv,gsttype,descript1,taxpcent,currency,rate,consign,your_ref) values (:ddate,:accountno,:sub,:accno,:br,:subbr,:debit,:credit,:reference,:inv,:gsttype,:descript1,:taxpcent,:currency,:rate,:consign,:your_ref)");
			$db->bind(':ddate', $ddate);
			...
			$totalvalue = $total;
				...
				$db->query("insert into ".$findb.".invhead (ddate,accountno,branch,sub,gldesc,transtype,ref_no,totvalue,tax,cash,cheque,eftpos,ccard,client,staff,currency,rate) values (:ddate,:accountno,:branch,:sub,:gldesc,:transtype,:ref_no,:totvalue,:tax,:cash,:cheque,:eftpos,:ccard,:client,:staff,:currency,:rate)");
				$db->bind(':ddate', $ddate);
				$db->bind(':accountno', $acc2cr);	
				$db->bind(':branch', $br2dr);
				$db->bind(':sub', $sub2cr);
	...
	if ($gstinvpay == 'Payment') {
			...
			if ($tax > 0) {
				...
				if ($acc2cr >= 751 && $acc2cr <= 800) {
					$db->query("insert into ".$findb.".trmain (ddate,accountno,branch,accno,br,debit,credit,reference,gsttype,gstrecon,grosspurchases,descript1,taxpcent,currency,rate,your_ref) values (:ddate,:accountno,:branch,:accno,:br,:debit,:credit,:reference,:gsttype,:gstrecon,:grosspurchases,:descript1,:taxpcent,:currency,:rate,:your_ref)");
					$db->bind(':ddate', $ddate);
