...
$where = "where h.accountno = ".$ac." and h.sub = ".$sb." and (substring(t.ref_no,1,3) = 'INV') and t.paid > 0 "; 
...
$grid->SelectCommand = "select t.uid, h.ddate,t.item,t.ref_no, t.paid from ".$findb.".invtrans t inner join ".$findb.".invhead h on t.ref_no = h.ref_no  ".$where;
...
$grid->setUrl('getallocinvs.php');
// Set grid caption using the option caption
$grid->setGridOptions(array(
    "caption"=>"Allocations against Sales",
    "rowNum"=>10,
    "sortname"=>"ddate",
	"sortorder" => 'desc',
...
function(rowid){
	var ids = jQuery("#unalinvlist").getDataIDs();
	for(var i=0;i<ids.length;i++){
			var rowd = $("#unalinvlist").getRowData(ids[i]);
			var cl = ids[i];
			...
			jQuery("#unalinvlist").setRowData(ids[i],{act:be}); 
	}
}
LOADCOMPLETE;
...
// Run the script
$grid->renderGrid('#unalinvlist','#unalinvpager2',true, null, null, true,true);
