...
function addjournal() {
	...
	$.get("includes/ajaxAddJournal.php", {account:n2dr, descript:note, debit:dramount, credit:cramount, accno:a2dr, subac:s2dr, brac:b2dr, ddate:ddate, reference:reference}
	...
	var x = 0, y = 0; // default values	
 	x = window.screenX +5;
  	y = window.screenY +265;
		...
		window.open('st_additem.php','addit','toolbar=0,scrollbars=1,height=670,width=1190,resizeable=yes,left='+x+',screenX='+x+',top='+y+',screenY='+y);
}
	...
	var x = 0, y = 0; // default values	
 	x = window.screenX +5;
  	y = window.screenY +265;
		...
		window.open('st_edititem.php?uid='+uid,'addit','toolbar=0,scrollbars=1,height=670,width=1190,resizeable=yes,left='+x+',screenX='+x+',top='+y+',screenY='+y);
}
...
function nexttrdref(ref) {
	...
	var dt = SQLdate(document.getElementById('newdate').value);
...
function addp_o() {
	...
	$.get("includes/ajaxAddTrdTrans.php", {stkid:stkid,stkitem:stkitem,stkprice:stkprice,stkunit:stkunit,stkqty:stkqty,tpcent:tpcent,taxindex:taxindex,taxtype:taxtype,sac:sac,sbr:sbr,ssb:ssb,pac:pac,pbr:pbr,psb:psb,grp:grp,cat:cat,priceband:priceband,discount:discount,disctype:disctype,setsell:setsell,loc:loc,stock:stock,avcost:avcost}, function(data){$("#trdpolist").trigger("reloadGrid")});
...
function postTrdTrans2(type) {
	...
	if (trading == 'crn') {
		$.get("includes/ajaxPostTrade.php", {type:type,ddate:ddate,descript:descript,ref:ref,acc:acc,asb:asb,loc:loc,postaladdress:postaladdress,deliveryaddress:deliveryaddress,clt:clt,paymethod:paymethod,staffmember:staffmember}, function(data){$("#purchlist").trigger("reloadGrid")});
	} else {
		$.get("includes/ajaxPostTrade.php", {type:type,ddate:ddate,descript:descript,ref:ref,acc:acc,asb:asb,loc:loc,postaladdress:postaladdress,deliveryaddress:deliveryaddress,clt:clt,paymethod:paymethod,staffmember:staffmember}, function(data){$("#tradlist").trigger("reloadGrid")});
	}
...
function downloadsql() {
	...
	$.get("../includes/bkup/ajaxgetdb.php", {}, function(data){
		var bkupfile = "../includes/bkup/"+data;
...
function statement() {
	...
	var curdate = SQLdate(document.getElementById('sdate').value);
	var lstdate = SQLdate(document.getElementById('edate').value);
...
function payfull(id,tp,refno) {
	...
	if (typ == 'pay') {
		$.get("includes/ajaxPayfull.php", {id: id, topay: topay, paymethod: paymethod, refno: refno, ddate:ddate}, 
...
function payfullp(id,topay,refno) {
	...
	$.get("includes/ajaxPayfullp.php", {id: id, topay: topay, refno: refno, ddate: ddate, payref: payref}, 
		  function(data){$("#outgrntrans").trigger("reloadGrid");
						 $("#outgrnlist").trigger("reloadGrid");
						});
	var aloc = parseFloat(unalloc - topay).toFixed(2);
...
function finddetails(typ) {
	...
	if (typ == 'ret') {
		...
		if (pref != 'GRN' && pref != 'C_P') {
			alert('Must be an goods recieved or cash purchase - GRN or C_P');
		ok = 'N';
			return false;
		}
...
function postcrn() {
	...
	$.get("includes/ajaxPostTrade.php", {type:trading,ddate:ddate,descript:descript,ref:ref,acc:acc,asb:asb,loc:loc,postaladdress:postaladdress,deliveryaddress:deliveryaddress,clt:clt,paymethod:paymethod,purchref:purchref},	function(data){$("#purchlist").trigger("reloadGrid")});
...
function postret() {
		...
		tradingref = ref;
		$.get("includes/ajaxtrimtrans.php", {}, function(data){});	
		$.get("includes/ajaxPostTrade.php", {type:trading,ddate:ddate,descript:descript,ref:ref,acc:acc,asb:asb,loc:loc,postaladdress:postaladdress,deliveryaddress:deliveryaddress,clt:clt,paymethod:paymethod,purchref:pref},	function(data){$("#purchlist").trigger("reloadGrid")});
...
//************************************************************************************************************
function gst() {
	var fdt = SQLdate(document.getElementById('fdate').value);
	var edt = SQLdate(document.getElementById('edate').value);
...
function det1gl() {
	var fdt = SQLdate(document.getElementById('bdate').value);
	var edt = SQLdate(document.getElementById('edate').value);
...
function viewdr2m() {
	var fdt = SQLdate(document.getElementById('bdate').value);
	var edt = SQLdate(document.getElementById('edateh').value);
