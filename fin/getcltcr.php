...
function(rowid){
			var ids = jQuery("#cltlistcr").getDataIDs(); 
			for(var i=0;i<ids.length;i++){ 
				var cl = ids[i]; 
				be = '<img src="../images/add.gif" title="Add as Creditor" onclick="javascript:add2cr('+cl+')" ></ids>'; 
				jQuery("#cltlistcr").setRowData(ids[i],{act:be}) 
			} 
}
LOADCOMPLETE;
...
$buttonoptions = array("#cltcrpager2",
    array("buttonicon"=>"ui-icon-plus","caption"=>"","position"=>"first","title"=>"Add a Client", "onClickButton"=>"js: function(){addclt('c');}")
);
$grid->callGridMethod("#cltlistcr", "navButtonAdd", $buttonoptions); 

// Run the script
$grid->renderGrid('#cltlistcr','#cltcrpager2',true, null, null, true,true);
