...
$db->query("select account,accountno,sub from ".$findb.".glmast where ctrlacc = 'N' and blocked = 'N' and system = 'N' and accountno >= 101 and accountno <= 200");
$rows = $db->resultset();
$cos_options = "<option value=\"\">Select COS Account</option>";
foreach ($rows as $row) {
	extract($row);
	$cos_options .= "<option value=\"".$accountno."~".$sub."\">".$account."</option>";
}
...
<?php
	if($_REQUEST['savebutton'] == "Y") {
	  ...
	  $db->query("insert into ".$findb.".".$chargefile." (supplier,acno,sbno,descript,charge,taxpcent,taxtype,cosacno,cossub,currency,rate) values (:supplier,:acno,:sbno,:descript,:charge,:taxpcent,:taxtype,:cosacno,:cossub,:currency,:rate)");
