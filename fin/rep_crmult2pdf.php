...
$vac = $_SESSION['s_viewcr1'];
$va = explode('~',$vac);
$ac = $va[0];
$sb = $va[1];
$nm = $va[2];
$vac2 = $_SESSION['s_viewcr2'];
$va2 = explode('~',$vac2);
$ac2 = $va2[0];
$sb2 = $va2[1];
$nm2 = $va2[2];
...
$wherecats = "";
$catcode = substr($_REQUEST['acat'], 0, strlen($_REQUEST['acat'])-1); 	
if ($catcode <> '') {
	$cat = explode(",",$catcode);
	foreach ($cat as $value) {
		$wherecs .= " or acats.category = '".$value."'";
	}
	$wherecs .= ")";
	$wherecats = substr($wherecs,3);
	$wherecats = " and (".$wherecats;
} else {
	$catcode = 'All';
}
...
$q = "select distinct concat(members.lastname,' ',members.firstname,' ',client_company_xref.subname) as account,client_company_xref.crno as accountno,client_company_xref.crsub as sub from members left join client_company_xref on members.member_id = client_company_xref.client_id left join acats on members.member_id = acats.member_id where client_company_xref.company_id = ".$coyidno." and client_company_xref.crno != 0 and (client_company_xref.member between '".$nm."' and '".$nm2."') and client_company_xref.blocked = 'No' ".$wherecats." order by client_company_xref.sortcode";
...
while ($row = mysql_fetch_array($r)) {
	...
	$heading = $account." - ".$sub." - between '".$fromdate."' and '".$todate."'";
...
$q2 = "select date_format(ddate,get_format(date,'EUR')) as dt,accno,br,subbr,format(debit,2) as db, format(credit,2) as cr, reference, descript1 from trmain where accountno = ".$a." and branch = '".$b."' and sub = ".$s." and ddate >= '".$fromdate."' and ddate <= '".$todate."' order by ddate" ;
	...
	if ($numrows > 0) {
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(175,5,'Categories:- '.$catcode,0,0,"L",1);
		$pdf->SetTextColor(0,0,0);
