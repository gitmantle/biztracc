...
$where = "where ".$cltdb.".client_company_xref.company_id = ".$coyidno." and ".$cltdb.".client_company_xref.drno != 0"; 
...
{
    if(rowid != null) {
		var rowd = $("#drlisting").getRowData(rowid);
		var draccno = rowd.drno;
		var drsubno = rowd.drsub;
		var dr = draccno+'~'+drsubno;
        jQuery("#drtranslist").jqGrid('setGridParam',{postData:{cid:dr}});
        jQuery("#drtranslist").trigger("reloadGrid");
    }
}
GRP;
...
function(rowid, selected)
{
   ...
   jQuery("#drtranslist").jqGrid('clearGridData',true);
}
CLEAR;
...
// Run the script
$grid->renderGrid('#drlisting','#drlistingpager2',true, null, null, true,true);
