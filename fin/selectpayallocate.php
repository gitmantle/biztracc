...
$db->execute();
$db->query("create table ".$findb.".".$crfile." (account varchar(45),priceband int,accountno int,Sub int default 0,client_id int default 0)  engine myisam");
$db->execute();

$db->query("insert into ".$findb.'.'.$crfile." select concat(members.lastname,' ',members.firstname,' ',client_company_xref.subname) as account,client_company_xref.priceband,client_company_xref.crno as accountno,client_company_xref.crsub as sub,client_company_xref.client_id from ".$cltdb.".members left join ".$cltdb.".client_company_xref on members.member_id = client_company_xref.client_id where client_company_xref.company_id = ".$coyidno." and client_company_xref.crno != '' and client_company_xref.blocked = 'No' ");
$db->execute();
...
$grid->SelectCommand = "select account,accountno,sub,client_id,priceband from ".$findb.'.'.$crfile.$where;
...
$grid->setUrl('selectpayallocate.php');
...
{
    if(rowid != null) {
	var rowdata = $("#selectpaylist").getRowData(rowid);
...
// Run the script
$grid->renderGrid('#selectpaylist','#selectpaypager',true, null, null, true,true);
