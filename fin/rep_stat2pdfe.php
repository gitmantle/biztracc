...
$dbp->query("select * from ".$findb.".".$drfile." where sendby = 'Email'");
$rows = $dbp->resultset();
foreach ($rows as $row) {
	extract($row);
	...
	$fname = 'trading_docs/'.$coyid.'/'."ste".$ac.$sb.".pdf";
	//$fn = "statementse.pdf";
	$pdf->Output($fname,'F');
	$pdf->close();
...
//******************************************************************************************************
	...
	if ($ok == 'Y') {
		...
		$mailclient = $_SESSION['s_mailclient'];
