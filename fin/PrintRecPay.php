...
$tradingref = $_REQUEST['tradingref'];
$totalvalue = $_REQUEST['amount'];
...
if ($type == 'REC') {
	$draccno = $_SESSION['s_drac'];
	$drsubno = $_SESSION['s_drsb'];
	$client = $_SESSION['s_clientname'];
	$qd = "select addresses.street_no,addresses.ad1 as line1,addresses.ad2 as line2,addresses.suburb,addresses.town,addresses.postcode from addresses,client_company_xref where addresses.member_id = client_company_xref.client_id and addresses.address_type_id = 4 and client_company_xref.drno = ".$draccno." and client_company_xref.drsub = ".$drsubno;
} else {
	$draccno = $_SESSION['s_crac'];
	$drsubno = $_SESSION['s_crsb'];
	$client = $_SESSION['s_crclientname'];
	$qd = "select addresses.street_no,addresses.ad1 as line1,addresses.ad2 as line2,addresses.suburb,addresses.town,addresses.postcode from addresses,client_company_xref where addresses.member_id = client_company_xref.client_id and addresses.address_type_id = 4 and client_company_xref.crno = ".$draccno." and client_company_xref.crsub = ".$drsubno;
}
...
$toadd = $client."\n".$postaladdress;
$gstabn = "";
$reference = $tradingref;

$recmessage = "Thank you for your payment of $".$totalvalue;
...
//***************************************************************************
{
	global $template, $pdf, $fromadd, $toadd, $gstabn, $reference, $orient, $paper, $ddate, $tradingref, $postaladdress, $deliveryaddress, $recmessage;
	...
	if ($include == 'Y') {
		...
		$pdf->Cell($cellwidth,$cellheight,$recmessage,$border,$nextpos,$align,$fill);
	}
...
//******************************************************************************
{
	global $template, $pdf, $lastpage, $currentline, $tradingref, $table;
	
	$q = "select refno,ddate,item,quantity,price,gst,total,paid from ".$table;
...
//********************************************************************************
{
	global $template, $pdf, $reference, $tradingref, $remmitance, $table, $totalvalue;
	
	//$q = "select sum(paid) as totalvalue from ".$table;
	//$r = mysql_query($q) or die (mysql_error().' '.$q);
	//$row = mysql_fetch_array($r);
	//extract($row);
