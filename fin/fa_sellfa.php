...
$db->query("select accountno,sub,branch,asset,cost,totdep from ".$findb.".fixassets where uid = ".$aid);
$row = $db->single();
extract($row);
...
function post() {
	...
	var CRaccountsList = document.getElementById('bgacc2cr').value;
	if(CRaccountsList == "") {
...
// debit asset realisation, credit fixed asset with purchase price of asset	
	$.get("includes/ajaxAddTrans.php", {acc2dr:90, subdr:0, brdr:b2cr, acc2cr:a2cr, subcr:s2cr, brcr:b2cr, ddate:ddate, descript1:description, reference:reference, amount:<?php echo $facost; ?>, taxpcent:0, tax:0, taxtype:'N-T', total:<?php echo $facost; ?>, refindex:refindex, taxindex:taxindex, a2d:n2dr, a2c:n2cr, drgst:drgst, crgst:crgst}, function(data){});
	
// debit accumulated depreciation, credit asset realisation with accumulated depreciation of asset
	$.get("includes/ajaxAddTrans.php", {acc2dr:702, subdr:0, brdr:b2cr, acc2cr:90, subcr:0, brcr:b2cr, ddate:ddate, descript1:'Accumulated depreciation on '+n2cr, reference:reference, amount:<?php echo $fadepn; ?>, taxpcent:0, tax:0, taxtype:'N-T', total:<?php echo $fadepn; ?>, refindex:refindex, taxindex:taxindex, a2d:n2dr, a2c:n2cr, drgst:drgst, crgst:crgst}, function(data){});

// credit asset realisation, debit bank, cash on hand or debtor with sale price of asset.
	$.get("includes/ajaxAddTrans.php", {acc2dr:a2dr, subdr:s2dr, brdr:b2dr, acc2cr:90, subcr:0, brcr:b2cr, ddate:ddate, descript1:description, reference:reference, amount:amount, taxpcent:taxpcent, tax:tax, taxtype:taxtype, total:total, refindex:refindex, taxindex:taxindex, a2d:n2dr, a2c:n2cr, drgst:drgst, crgst:crgst}, function(data){});
	
	//$.get("includes/ajaxSellAsset.php", {}, function(data){});
...
<form name="rtrans" id="rtrans" method="post" >
  ...
  <table width="950" border="0" cellpadding="3" cellspacing="1" align="center" bgcolor="<?php echo $bgcolor; ?>">
    <tr bgcolor="<?php echo $bghead; ?>">
      <td colspan="5" align="center"><label style="color: <?php echo $thfont; ?>"><strong>Sell an Asset </strong></label></td>
    </tr>
    <tr>
...
<input type="Text" id="newdate" name="newdate" value="<?php echo $ddateh; ?>" class="flatpickr" data-alt-input="true" data-alt-format="F j,Y"></div></td>
      ...
      <td ><select name="ref" id="refno" onchange="ajaxGetRef(this.value); return false;">
          ...
          <option value="EBI">Electronic Banking</option>
    ...
    <tr bgcolor="#CCCCCC">
      ...
      <td><div align="left">
          <select name="bgacc2dr" id="bgacc2dr" onchange="ajaxGetACList(this.value,'dr'); return false;">
    ...
    <tr bgcolor="#FF9933">
      <td class="boxlabel"><label style="color:#000000;">Asset to Sell</label></td>
      <td><div align="left">
          <input type="text" name="bgacc2cr" id="bgacc2cr" size="45" value="<?php echo $faselected; ?>" readonly >
        </div></td>
      <td colspan="3"><div align="left">
  ...
  <div id="glselect" style="position:absolute;visibility:hidden;top:104px;left:455px;height:239px;width:451px;background-color:<?php echo $bgcolor; ?>;border-width:thick thick thick thick; border-color:<?php echo $bghead; ?>; border-style:solid;">
    <table>
      ...
      <tr>
        <td colspan="2"><?php include "selectbankd.php"; ?></td>
      </tr>
    </table>
  </div>
