...
$db->query("select Header, AccountNumber, Branch, Sub, AccountName, Col1,Col2,Total from ".$findb.".".$bsfile);
$rows = $db->resultset();
$count = count($rows);

$heading = $coyname.' - '.$_SESSION['s_bsheading'];
...
foreach ($rows as $row){
	...
	$cell = 'F'.$i;
	$objPHPExcel->getActiveSheet()->setCellValue($cell, $Col1);	
	$cell = 'G'.$i;
	$objPHPExcel->getActiveSheet()->setCellValue($cell, $Col2);	
	$cell = 'H'.$i;
...
header('Content-Disposition: attachment;filename="bs.xlsx"');
