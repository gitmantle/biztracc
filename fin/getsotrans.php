...
$grid->callGridMethod('#solist', 'footerData', array("set",array("quantity"=>"Total: ".$localcurrency)));
...
function(rowid){
	...
	for(var i=0;i<ids.length;i++){
			...
			jQuery("#solist").setRowData(ids[i],{act:be+'&nbsp;&nbsp;&nbsp;'+se}); 
	}
}
LOADCOMPLETE;
...
$buttonoptions2 = array("#sopager",
    array("buttonicon"=>"ui-icon-calculator","caption"=>"","position"=>"last","title"=>"Calculators", "onClickButton"=>"js: function(){showCalculators();}")
);
$grid->callGridMethod("#solist", "navButtonAdd", $buttonoptions2); 

// Run the script
$grid->renderGrid('#solist','#sopager',true, $summaryrows, null,true,true,true);
$conn = null;
