...
$user = "logtracc9";
$pwd = "dun480can";
$connect = mysql_connect($host,$user,$pwd) or die("Check your database connection - db");
...
if ($type <> 'REC' && $type <> 'PAY') {
	...
	extract($row);
	$deliveryaddress = "";
	$postaladdress = trim($street_no.' '.$ad1)."\n";
	$postaladdress .= trim($ad2.' '.$suburb)."\n";
	$postaladdress .= trim($town.' '.$postcode);
	
	$toadd = 'client '.$postaladdress;
	$gstabn = "gstno";
	$rference = "reference";
...
//******************************************************************************
{
	...
	$q = "select itemcode,item,quantity,price,value,discount,tax,format(value+tax,2) as total from invtrans where ref_no = '".$tradingref."'";
