...
$db->query("select subname from ".$cltdb.".client_company_xref where client_id = ".$mid." and crsub = ".$sno);
$row = $db->single();
extract($row);
$sname = $subname;
...
<br>
  <table width="600" border="0" align="center">
    <tr>
      <td colspan="2"><div align="center" class="style1 style3"><u>Edit a Creditor Sub Account</u></div></td>
    </tr>
    <tr>
...
<?php
	...
	if ($ok == 'Y') {
		...
		$db->query("update ".$cltdb.".client_company_xref set subname = '".$sname."' where client_id = ".$mid." and crsub = ".$sno);
		$db->execute();
		$db->closeDB();
