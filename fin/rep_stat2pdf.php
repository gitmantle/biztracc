...
//******************************************************************************
{
	global $template, $pdf, $lastpage, $currentline, $ac, $sb, $currentline, $lastpage, $fdate, $tdate;
	...
	$q = "select ddate, ref_no, gldesc, totvalue, tax, (totvalue + tax) as total from invhead where accountno = ".$ac." and sub = ".$sb." and ddate > '".$fdate."' and ddate <= '".$tdate."'";
	...
	if ($include == 'Y') {
		...
		$pdf->Cell($cellwidth,$cellheight,$obal,$border,$nextpos,$align,$fill);
	}
...
$fname = 'trading_docs/'.$coyid.'/'."statementse.pdf";
$fn = "statementse.pdf";
//$pdf->Output($fname,'F');
//$pdf->Output($fn,'I');
//$pdf->close();
...
$fname = 'trading_docs/'.$coyid.'/'."statementsp.pdf";
$fn = "statementsp.pdf";
$pdf->Output($fname,'F');
$pdf->Output($fn,'I');
$pdf->close();
