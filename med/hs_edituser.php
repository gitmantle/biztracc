...
$query = "select users.ufname,users.ulname,users.coyowner,users.uadmin,users.uemail,users.uphone,users.umobile,users.ug_user,users.ug_pwd,users.uadmin,users.depot,access.usergroup from users,access where users.uid = access.staff_id and users.uid = ".$uid; 
$result = mysql_query($query) or die(mysql_error());
$row = mysql_fetch_array($result);
extract($row);
$userid = $uid;
