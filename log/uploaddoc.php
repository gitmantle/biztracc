 ...
 */
function upload($file_id, $folder="", $types="", $sid, $staffid) {
    ...
    $file_name = $staffid.'__'.$file_title;//Get Unique Name
    ...
    if($folder) $folder .= '/'.$sid.'/';//Add a '/' at the end of the folder
...
if($_FILES['image']['name']) {
	...
	list($file,$error) = upload('image','documents','jpeg,gif,png,doc,xls,pdf,docx,xlsx,jpg,ppt,rtf,txt',$sid,$staffid);
	if($error) {
		print $error;
	} else {
		...
		$query = "insert into documents (staffid,title,document) values ";
		$query .= "(".$staffid.",";
		$query .= "'".$filetitle."',";
		$query .= "'".$file."')";
		...
		$result = mysql_query($query) or die(mysql_error().' '.$query);
		 ...
		 echo  "<script>";
		   echo 'window.open("","docgrid").jQuery("#doclist").trigger("reloadGrid");';
		 echo 'this.close();';
		  echo '</script>';
...
<tr>
<th align="center"><label style="color: <?php echo $tdfont; ?>">Select signed PDF document to Upload</label></th>
</tr>
<tr>
