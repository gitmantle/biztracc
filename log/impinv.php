...
function upload($file_id, $folder="", $types="",$sid,$cid) {
    ...
    $file_name = 'docinv_'.$sid . '__'.$cid.'.'.$ext;//Get Unique Name
    ...
    if (!move_uploaded_file($_FILES[$file_id]['tmp_name'], $uploadfile)) {
        ...
        if(!file_exists($folder)) {
            $result .= $folder." : Folder does not exist.";
...
if($_FILES['image']['name']) {
	...
	list($file,$error) = upload('image','imports','xlsx',$subscriber,$coyid);
	if($error) {
	  print $error;
	} else {
	  echo '<script>';
	  ...
	  echo "window.open('loadinvs.php','ast','toolbar=0,scrollbars=1,height=460,width=820,resizable=1,left='+x+',screenX='+x+',top='+y+',screenY='+y);";
	  echo 'this.close();';
	  echo '</script>';
	}
}
...
<title>Import Docket Invoice Details</title>
...
<table width="800" border="0" align="center" bgcolor="<?php echo $bgcolor; ?>">
    <tr>
    	<td align="center"><label style="color: <?php echo $tdfont; ?>; font-size:14px;">Please ensure the spreadsheet you obtained is Excel2007 format (.xlsx) </label></td>
    </tr>
    <tr>
