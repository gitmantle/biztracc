...
function upload($file_id, $folder="", $types="", $sid) {
    ...
    if($folder) $folder .= '/'.$sid.'/templates'.'/';//Add a '/' at the end of the folder
...
if($_FILES['image']['name']) {
	...
	if($error) {
		...
		$query = "insert into templates (title,template) values ";
		$query .= "('".$filetitle."',";
		$query .= "'".$file."')";
		...
		$result = mysql_query($query) or die(mysql_error().' '.$query);
		 ...
		 echo  "<script>";
		   echo 'window.open("","templates").jQuery("#templatelist").trigger("reloadGrid");';
		 echo 'this.close();';
		  echo '</script>';
...
<tr>
<th align="center"><label style="color: <?php echo $tdfont; ?>">Select Template PDF document to Upload</label></th>
</tr>
<tr>
  <td align="center">Template Title
    <input type="text" name="title" id="title" size="50"/></td>
</tr>
<tr>
