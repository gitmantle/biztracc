...
$ddate = date("Y-m-d");
	...
	$query = "select uid,incbasic from incbasic order by incbasic";		
	$result = mysql_query($query) or die(mysql_error());

	$basics = array();
	if(mysql_num_rows($result)) {
		while($basic = mysql_fetch_assoc($result)) {
		  $basics[] = array('cause'=>$basic);
		}
		
		header('Content-type: application/json');
		echo json_encode(array('basics'=>$basics));
	} else {
