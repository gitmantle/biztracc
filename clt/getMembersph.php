...
if(isset($_REQUEST["ph_mask"])) {
	$ph_mask = $_REQUEST['ph_mask'];
} else {
  	$ph_mask = ""; 
}
...
$where = " WHERE 1 = 1"; 
if($ph_mask!='') {
	$where.= " AND members.member_id = comms.member_id and instr(comms.comm2, '".$ph_mask."')"; 
}
...
$grid->SelectCommand = "select members.member_id,members.lastname,members.firstname,members.preferredname,members.dob,members.checked,comms.area_code,comms.comm from ".$cltdb.".members,".$cltdb.".comms".$where." group by member_id";
