...
$assoctype = $_REQUEST['assoctype'];
$isdependant = $_REQUEST['isdependant'];
...
function addreferred(mp) {
	if (mp == 'm') {
		...
		var m = document.getElementById('preferred');
		var listlengthm = document.getElementById("preferred").length;
		var newref = document.getElementById('pref').value;
	}
	if (newref != '') {
		$.get("addreferred.php", {newref: newref}, function(data){
			document.getElementById('mreferred').add(new Option(newref,data), null);
			document.getElementById('preferred').add(new Option(newref,data), null);
...
function post() {
	...
	if (ok == "Y") {
		...
		document.getElementById('addassociation').submit();
	}
...
function pgetpc() {
	var adloc = "Street";
	var adtype = 1;
	var stno = document.getElementById('pstreetno').value;
	var sad1 = document.getElementById('pstreet').value;
	
	ajaxGetphPostCode(adloc,adtype,stno,sad1," "," "," "," "," ");
...
function ppopads(ad) {
	var addr = ad.split('~');
	document.getElementById('pstreet').value = addr[1];
	document.getElementById('psuburb').value = addr[2];
	document.getElementById('ptown').value = addr[3];
	document.getElementById('ppostcode').value = addr[4];
}
...
<?php
	if($_REQUEST['savebutton'] == "Y") {
		...
		$newuid = $oCn->AddMember();
		
		$oCn->client_id = $newuid;
		...
		$q = "insert into assoc_xref (member_id,association,of_id,sub_id) values (".$associd.",'".$revassoc."',".$memid.",".$subid.")";
		$r = mysql_query($q) or die(mysql_error().$q);
		$q = "insert into assoc_xref (member_id,association,of_id,sub_id) values (".$memid.",'".$assoctype."',".$associd.",".$subid.")";
		$r = mysql_query($q) or die(mysql_error().$q);

		if ($isdependant == 'Yes') {
			$q = "update members set dependant = 'Y' where member_id = ".$associd;
			$r = mysql_query($q) or die(mysql_error().$q);
		} else {
			$q = "update members set dependant = 'N' where member_id = ".$associd;
			$r = mysql_query($q) or die(mysql_error().$q);
		}
