...
function(rowid) {
	var rowdata = $("#ccemaillist").getRowData(rowid);
	var recip = rowdata.recipient;
	addcc(rowid,recip);
	
	$(".HeaderButton", $('#ccemaillist')[0].grid.cDiv).trigger("click");	
...
$buttonoptions = array("#ccemailpager",
    array("buttonicon"=>"ui-icon-plus","caption"=>"","position"=>"first","title"=>"Add an email address", "onClickButton"=>"js: function(){addrecipient();}")
);
$grid->callGridMethod("#ccemaillist", "navButtonAdd", $buttonoptions); 

// Run the script
$grid->renderGrid('#ccemaillist','#ccemailpager',true, null, null, true,true);
