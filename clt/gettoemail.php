...
$grid->SelectCommand = "SELECT subemail_id,recipient,email from ".$cltdb.".subemails";
...
function(rowid) {
	var rowdata = $("#toemaillist").getRowData(rowid);
	var em = rowdata.email;
	addto(rowid,em);
	
	$(".HeaderButton", $('#toemaillist')[0].grid.cDiv).trigger("click");	
...
$buttonoptions = array("#toemailpager",
    array("buttonicon"=>"ui-icon-plus","caption"=>"","position"=>"first","title"=>"Add an email address", "onClickButton"=>"js: function(){addrecipient();}")
);
$grid->callGridMethod("#toemaillist", "navButtonAdd", $buttonoptions); 

// Run the script
$grid->renderGrid('#toemaillist','#toemailpager',true, null, null, true,true);
