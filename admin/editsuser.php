...
$db->query("select users.ufname,users.ulname,users.coyowner,users.uadmin,users.uemail,users.uphone,users.umobile,users.ug_user,users.ug_pwd from users where users.uid = ".$uid); 
$row = $db->single();
extract($row);
...
<?php
	if(isset($_POST['save'])) {
		...
		if ($_REQUEST['chguser'] == "Y") {
			$db->query("update users set username = :uname where uid = :uid");
			$db->bind(':uname', md5($_REQUEST['un']));
			$db->bind(':uid', $uid);
			$db->execute();
		}
