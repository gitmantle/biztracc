...
$rounding = 0;
	...
	$db->query("insert into ".$findb.".invhead (ddate,accountno,branch,sub,gldesc,transtype,ref_no,xref,totvalue,tax,cash,cheque,eftpos,ccard,staff,postaladdress,deliveryaddress,client,signature) values (:ddate,:accountno,:branch,:sub,:gldesc,:transtype,:ref_no,:xref,:totvalue,:tax,:cash,:cheque,:eftpos,:ccard,:staff,:postaladdress,:deliveryaddress,:client,:signature)");
	$db->bind(':ddate', $ddate);
	...
	foreach ($rows as $row) {
		...
		$db->query("insert into ".$findb.".invtrans (itemcode,item,price,quantity,unit,taxtype,taxpcent,tax,ref_no,value,discount,disc_type,grnlineno) values (:itemcode,:item,:price,:quantity,:unit,:taxtype,:taxpcent,:tax,:ref_no,:value,:discount,:disc_type,:grnlineno)");
	...
	if ($rounding != 0) {
		...
		$db->query("insert into ".$findb.".invtrans (itemcode,item,price,quantity,unit,taxtype,taxpcent,tax,ref_no,value,discount,disc_type,grnlineno) values (:itemcode,:item,:price,:quantity,:unit,:taxtype,:taxpcent,:tax,:ref_no,:value,:discount,:disc_type)");
		$db->bind(':itemcode', $itemcode);
		...
		foreach ($rows as $row) {
			...
			if ($tax > 0 || $transtype == 'GRN') {
				...
				if ($gstinvpay == 'Invoice') {
					if ($transtype == 'INV' || $transtype == 'C_S') {
						$db->query('update '.$findb.'.trmain set grosssales = '.$tot.' where uid = '.$addedrec);
						$db->execute();							
					}
					if ($transtype == 'GRN' || $transtype == 'C_P') {
						$db->query('update '.$findb.'.trmain set grosspurchases = '.$tot.' where uid = '.$addedrec);
						$db->execute();							
					}
					if ($transtype == 'CRN') {
						$tot = $tot * -1;
		...
		if ($transtype == 'INV' || $transtype == 'C_S' || $transtype == 'REQ' || $transtype == 'RET') {
			...
			if ($transtype == 'GRN' || $transtype == 'C_P') {
				if ($value > 0) {
					...
					$newavgcost = number_format($newavgcost,2);
	...
	if ($dn2inv == 'Y') {
		...
		foreach ($rows as $row) {
		    ...
		    $db->query("update ".$findb.".stktrans set xref = ref_no, transtype = 'INV' where ref_no = ".$dn);
			$db->execute();
