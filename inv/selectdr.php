...
$query = "create table ".$drfile." (account varchar(45),priceband int,accountno int,Sub int default 0,client_id int default 0)  engine myisam";
$calc = mysql_query($query) or die(mysql_error());

$cltdb = $_SESSION['s_cltdb'];
mysql_select_db($cltdb) or die(mysql_error());

$q = "insert into ".$findb.'.'.$drfile." select concat(members.lastname,' ',members.firstname,' ',client_company_xref.subname) as account,client_company_xref.priceband,client_company_xref.drno as accountno,client_company_xref.drsub as sub,client_company_xref.client_id from members left join client_company_xref on members.member_id = client_company_xref.client_id where client_company_xref.company_id = ".$coyidno." and client_company_xref.drno != '' and client_company_xref.blocked = 'No' ";
...
$grid->SelectCommand = "select uid,account,accountno,sub,client_id,priceband from ".$drfile.$where;
...
function(rowid, selected)
{
	...
	var acc = ac+'~'+' '+'~'+sb+'~'+acname+'~'+clientid+'~'+priceband;
	setselect(acc,'dr');	
    }
}
ACC;
