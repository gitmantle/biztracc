...
$db->query("select * from ".$crndb.".vehicles where vehicleno = '".$vn."'");
$row = $db->single();
extract($row);
$cd = explode('-',$cofdate);
$cof = $cd[2].'/'.$cd[1].'/'.$cd[0];
...
$db->query("select * from ".$crndb.".serviceb where service_id = ".$sid);
$row = $db->single();
extract($row);
$sd = explode('-',$ddate);
...
<form name="form1" id="form1" method="post" >
 ...
 <table width="970" border="0" align="center" cellspacing="4" bgcolor="<?php echo $bgcolor; ?>">
    ...
    <tr>
      <td class="boxlabelleft">Engine oil &amp; filter</td>
      <td class="boxlabelleft"><select name="replace1" id="replace1">
    ...
    <tr>
      <td class="boxlabelleft">Fuel filter</td>
      <td class="boxlabelleft"><select name="replace2" id="replace2">
...
<?php
	...
	if($_REQUEST['savebutton'] == "Y") {
		...
		$q = "update ".$crndb.".serviceb set ";
		$q .= "check1 = '".$check1."',check2 = '".$check2."',check3 = '".$check3."',check4 = '".$check4."',check5 = '".$check5."',check6 = '".$check6."',check7 = '".$check7."',";
		...
		$q .= "replace1 = '".$replace1."',replace2 = '".$replace2."',";
		...
		$db->query("update ".$crndb.".vehicles set servicedue = ".$nextdue." where vehicleno = '".$vn."'");
		$db->execute();
		$db->query("update ".$crndb.".service set servicedue = ".$nextdue." where uid = '".$sid."'");
		$db->execute();
