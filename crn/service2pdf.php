...
$db->query("select * from ".$crndb.".service where uid = ".$svrid);
$row = $db->single();
extract($row);
switch ($service_type) {
	case 'A':
...
$db->query("select * from ".$crndb.".vehicles where vehicleno = '".$svehicleno."'");
$row = $db->single();
extract($row);
$cd = explode('-',$cofdate);
$cof = $cd[2].'/'.$cd[1].'/'.$cd[0];
...
$db->query("select * from ".$crndb.".".$dbs." where service_id = ".$svrid);
$row = $db->single();
extract($row);
$dt = explode('-',$ddate);
