...
if(isset($_REQUEST["coyname_mask"]) && $_REQUEST["coyname_mask"] != '' ) {
	if ($_REQUEST["notcoyname_mask"] == 'N') {
		$coyname_mask = " and coyid = '".$_REQUEST['coyname_mask']."'";
	} else {
		$coyname_mask = " and coyid != '".$_REQUEST['coyname_mask']."'";
	}
} else {
...
$q = "update ".$filterfile." set include = 'Y' ".$where;
$r = mysql_query($q) or die(mysql_error());

//construct the select statement
$sel = "select uid,incid,coyname,date_incident,latitude,longitude,road,incident_type,harm_people,damage_property from ".$filterfile;
