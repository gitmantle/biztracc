...
$q = "select uid,date_format(date_incident,'%d %M %Y') as dt_inc,latitude,longitude,coyid from ".$incfile." where date_incident >= '".$bdate."' and date_incident <= '".$edate."'";
...
while ($row = mysql_fetch_array($r)) {
	...
	mysql_select_db($moduledb) or die(mysql_error());
	$qc = "select coyname from companies where coyid = ".$coyid; 
	...
	$x = $x + 1;
	$inclocs .= "['".$dt_inc."', ".$latitude.", ".$longitude.", ".$x.", '".$dt_inc." Incident ".$uid." ".$cname."'],";
}
