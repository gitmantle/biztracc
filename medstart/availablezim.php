...
if($val == FALSE) {
	$query = "create table ".$avtable." (uid integer(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, guid varchar(50), item varchar(100), cost decimal(16,2) default 0, unit varchar(20), noinunit decimal(9,2), gitem varchar(100), gcost decimal(16,2) default 0, gunit varchar(20), gnoinunit decimal(9,2), xref int(11), entered date, taxtype int(11) default 0) engine myisam"; 
...
$q = "insert into ".$avtable." (guid, item, cost, unit, noinunit, xref, entered, taxtype) select '".$sessionid."',item,case setsell when 0 then (avgcost * ".$markup.") else setsell end as cost,unit,noinunit,xref,'".$today."',deftax from stkmast where generic = 'N' and active = 'Yes'";
...
while ($row = mysql_fetch_array($r)) {
	...
	$xr = $xref;
	if ($xr > 0) {
		$qs = "select item,unit,noinunit,case setsell when 0 then (avgcost * ".$markup.") else setsell end as cost from stkmast where itemid = ".$xr;
...
$q = "select uid, taxtype from ".$avtable;
$r = mysql_query($q) or die(mysql_error().' '.$q);
while ($row = mysql_fetch_array($r)) {
	extract($row);
	$ttyp = $taxtype;
	$sid = $uid;
	$qt = "select taxpcent from taxtypes where uid = ".$ttyp;
	$rt = mysql_query($qt) or die(mysql_error());
	$rowt = mysql_fetch_array($rt);
	extract($rowt);	
	$tpcent = $taxpcent;
	$qi = "update ".$avtable." set gcost = (gcost * (1 + (".$tpcent."/100))), cost = (cost * (1 + (".$tpcent."/100))) where uid = ".$sid;
	$ri = mysql_query($qi) or die(mysql_error().' '.$qi);
}
